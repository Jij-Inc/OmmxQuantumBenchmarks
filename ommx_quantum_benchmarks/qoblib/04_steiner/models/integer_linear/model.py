...
def create_steiner_tree_packing_model() -> jm.Problem:
    ...
    def _(problem: jm.DecoratedProblem):
        ...
        nodes_without_roots = problem.Natural("VNR", ndim=1, description="Vertices excluding root nodes")
        ...
        problem += problem.Constraint(
            ...
            lambda t, r: jm.sum(x[a, t] for a in nA if arcs[a, 0] == roots[r]) >= z[r, t],
            domain=jm.product(nT, nR),
        )

        problem += problem.Constraint(
            "root_flow_out_big_m_upper",
            lambda t, r: jm.sum(x[a, t] for a in nA if arcs[a, 0] == roots[r]) <= z[r, t] + big_m * (1 - z[r, t]),
            domain=jm.product(nT, nR),
        )

        problem += problem.Constraint(
            "root_flow_out_z_condition",
            lambda t, r: z[r, t] <= 1 - jm.abs(terminal_innet[t, 1] - root_innet[r, 1]) / big_m,
            domain=jm.product(nT, nR),
        )
        ...
        problem += problem.Constraint(
            "root_flow_in",
            lambda t, r: jm.sum(x[a, t] for a in nA if arcs[a, 1] == roots[r]) == 0,
            domain=jm.product(nT, nR),
        )
        ...
        problem += problem.Constraint(
            "terms_flow_out",
            lambda t: jm.sum(x[a, t] for a in nA if arcs[a, 0] == terminals[t]) == 0,
            domain=nT,
        )
        ...
        problem += problem.Constraint(
            "terms_flow_in",
            lambda t: jm.sum(x[a, t] for a in nA if arcs[a, 1] == terminals[t]) == 1,
            domain=nT,
        )
        ...
        problem += problem.Constraint(
            "terms_flow_bal_same",
            lambda t, s: jm.sum(x[a, t] for a in nA if arcs[a, 1] == terminals[s]) - jm.sum(x[a, t] for a in nA if arcs[a, 0] == terminals[s]) == 0,
            domain=jm.product(nT, nT).filter(lambda t, s: (terminals[s] != terminals[t]) & (terminal_innet[s, 1] == terminal_innet[t, 1])),
        )
        ...
        problem += problem.Constraint(
            "terms_flow_bal_diff",
            lambda t, s: jm.sum(x[a, t] for a in nA if arcs[a, 1] == terminals[s]) - jm.sum(x[a, t] for a in nA if arcs[a, 0] == terminals[s]) == 0,
            domain=jm.product(nT, nT).filter(lambda t, s: terminal_innet[s, 1] != terminal_innet[t, 1]),
        )

        # 7. NORMAL NODES FLOW BALANCE
        problem += problem.Constraint(
            "normal_flow_bal",
            lambda t, nn: jm.sum(x[a, t] for a in nA if arcs[a, 0] == normals[nn]) - jm.sum(x[a, t] for a in nA if arcs[a, 1] == normals[nn]) == 0,
        ...
        problem += problem.Constraint(
            "bind_x_y",
            lambda a, l: jm.sum(x[a, t] for t in nT if terminal_innet[t, 1] == nets[l]) <= net_cardinality[l, 1] * y[a, nets[l]],
        ...
        problem += problem.Constraint(
            "disjoint_nonroot",
            lambda nwr: jm.sum(y[a, nets[l]] for a in nA for l in nL if arcs[a, 1] == nodes_without_roots[nwr]) <= 1,
            domain=nVNR,
        )
        ...
        problem += problem.Constraint(
            "disjoint_root",
            lambda r: jm.sum(y[a, nets[l]] for a in nA for l in nL if arcs[a, 1] == roots[r]) <= 0,
            domain=nR,
        )
