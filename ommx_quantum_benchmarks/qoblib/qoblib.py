...
_PORTFOLIO_UQO_LAMBDAS: Final[tuple[str, ...]] = (
    "0",
    "0.000001",
    "0.00001",
    "0.00005",
...
def _portfolio_uqo_available_instances() -> list[str]:
    return [
        f"uqo_a{num_assets:03d}_t{num_periods:02d}_{seed}_b{b_total:03d}_l{lam}"
        for num_assets, b_total in ((10, 4), (50, 20))
        for num_periods in (10, 15)
        for seed in ("orig", "s00", "s01", "s02")
        for lam in _PORTFOLIO_UQO_LAMBDAS
    ]
...
class Portfolio(BaseDataset):
    ...
    available_instances: dict[str, list[str]] = field(
        default_factory=lambda: {
            "binary_quadratic_programming": [],
            "unconstrained_quadratic_optimization": (
                _portfolio_uqo_available_instances()
            ),
        }
    )
