...
def create_topology_model() -> jm.Problem:
    ...
    Returns:
        ...
        defined for the topology problem using seidel quadratic approach.
    """
    ...
    # Constraint 2: DistCalc - quadratic distance calculation constraint
    # forall <k> in D without {max_d-1} : forall <s,t> in F : 
    # dist[s,t,k+1] <= dist[s,t,k] + sum <i> in N without {s,t} : 
    # dist[min(s,i),max(s,i),k] * dist[min(i,t),max(i,t),0]
    
    problem += jm.Constraint(
        "DistCalc",
        dist[s, t, k + 1] <= dist[s, t, k] + jm.sum(
            [(i, (i != s) & (i != t))], 
            dist[jm.min(s, i), jm.max(s, i), k] * dist[jm.min(i, t), jm.max(i, t), 0]
        ),
        forall=[s, (t, s < t), (k, k < max_d - 1)]
    )
    
    # Constraint 3: degreeButLast - degree constraint for all nodes except the last
    # forall <j> in N without {n-1} : sum <t> in N without {j}: dist[min(j,t),max(j,t),0] == d
    problem += jm.Constraint(
        "degreeButLast",
        jm.sum([(t, t != j)], dist[jm.min(j, t), jm.max(j, t), 0]) == d,
        forall=[(j, j != n - 1)]
    )
    
    # Constraint 4: degreeLast - degree constraint for the last node
    # sum <t> in N without {n-1} : dist[t,n-1,0] == if ((n * d) mod 2 == 0) then d else d-1 end
    # This constraint can be simplified to: d - (n*d mod 2)
    problem += jm.Constraint(
        "degreeLast",
        jm.sum([(t, t != n - 1)], dist[t, n - 1, 0]) == d - ((n * d) % 2),
        forall=[]
    )
