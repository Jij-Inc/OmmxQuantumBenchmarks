...
def create_problem():
    ...
    z = jm.BinaryVar("z", shape=(I_set.shape[0], K_set.shape[0]), description="Binary variable z")
    ...
    first_term = jm.sum(k, 
        (jm.sum(i, 4 * z[i, k] - 2 * x[i] - 2 * x[i + k + 1] + 1)) ** 2
    )

    second_term = P * jm.sum(k,
        jm.sum(i, 3 * z[i, k] - 2 * z[i, k] * x[i] 
            - 2 * z[i, k] * x[i + k + 1] + x[i] * x[i + k+1])
    )
